...
| 阶段 5：Gazebo Classic 11 仿真接入（4 插件 + 空世界 + 运动验收 3/3） | ✅ 完成 |
| 阶段 6：自建世界（simple 10×8 / complex 16×12，纯 SDF 零 Fuel） | ✅ 完成 |
| 阶段 7：接口与 TF 验收（9 topic + 频率 + TF + 遥控套件 7/7） | ✅ 完成 |
| 阶段 8：SLAM Toolbox 建图（50/50 航点闭环，地图 322×241） | ✅ 完成 |
| 阶段 9：AMCL + Navigation2 自主导航 | ✅ 完成（长路线成功率局限已记录） |
| 阶段 10：Nav2 参数调优 | ✅ 完成（3 轮 × 5 目标） |
| 阶段 11：一键 Bringup | ✅ 完成 |
| 阶段 12：Cartographer 建图 | ✅ 完成 |
| 阶段 13：公平对比实验 | ✅ 完成（同一 rosbag、8 项指标、30 次试验） |
| 阶段 14：最终交付（文档 + 检查清单） | ✅ 完成 |
